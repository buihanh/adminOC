...
class ControllerAccountDownload extends Controller {
	...
	public function info() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/download/info', '', 'SSL');
			$this->redirect($this->url->link('account/login', '', 'SSL'));
		}
		$this->language->load('account/download');
		$this->data['breadcrumbs'] = array();
		...
		$this->data['breadcrumbs'][] = array(
			'text'      => $this->language->get('text_downloads'),
			'href'      => $this->url->link('account/download/info', '', 'SSL'),       	
			'separator' => $this->language->get('text_separator')
		);
		...
		foreach ($results as $result) {
			...
			$resultss = $this->model_account_order->getOrderHistories($result['order_id']);	
			$order_status_id = $resultss[count($resultss)-1]['order_status_id'];
			foreach ($products as $product) {
				$product_file = 	$this->model_account_order->getProducts_My($product['product_id']);
				...
				$this->data['orders'][] = array(
					...
					'filesoure'   => $product_file[0]['filesoure'],
					'image' => $this->model_tool_image->resize($product_file[0]['image'], 100,100),
					'href' =>$this->url->link('product/download', 'product_id=' . $product['product_id'].'&order_id=' . $result['order_id'], 'SSL')
				);
			}
		}
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/download_info.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/account/download_info.tpl';
			} else {
				$this->template = 'default/template/account/download_info.tpl';
			}
