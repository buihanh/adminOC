...
class ControllerCommonFileManager extends Controller {
	...
	public function url_user() {
		return  'data/public/'.$this->customer->getEmail().'/';
	...
	public function index() {
		...
		$this->data['folder'] = $this->url_user();
		...
		$this->data['token'] = "264526546256452";//$this->session->data['token'];

		$this->data['directory'] = HTTP_CATALOG . 'image/'.$this->url_user();
	...
	public function images() {
		...
		$temp =  $this->url_user().$this->request->get['image'];
		echo $temp;
		//echo $this->model_tool_image->resize(html_entity_decode($temp, ENT_QUOTES, 'UTF-8'), 100, 100);
		die();
		if (isset($this->request->get['image'])) {
			$this->response->setOutput($this->model_tool_image->resize(html_entity_decode($temp, ENT_QUOTES, 'UTF-8'), 100, 100));
		}
	}
	...
	public function directory() {
		...
		if (isset($this->request->post['directory'])) {
			$directories = glob(rtrim(DIR_IMAGE .$this->url_user(). str_replace('../', '', $this->request->post['directory']), '/') . '/*', GLOB_ONLYDIR);
			...
			if ($directories) {
				...
				foreach ($directories as $directory) {
					...
					$json[$i]['attributes']['directory'] = utf8_substr($directory, strlen(DIR_IMAGE .$this->url_user()));
	...
	public function files() {
		...
		if (!empty($this->request->post['directory'])) {
			$directory = DIR_IMAGE .$this->url_user() . str_replace('../', '', $this->request->post['directory']);
		} else {
			$directory = DIR_IMAGE .$this->url_user();
		}
			...
			foreach ($files as $file) {
				...
				if (in_array(strtolower($ext), $allowed)) {
					...
					$json[] = array(
						...
						'file'     => utf8_substr($file, utf8_strlen(DIR_IMAGE .$this->url_user())),
	...
	public function create() {
		...
		if (isset($this->request->post['directory'])) {
			if (isset($this->request->post['name']) || $this->request->post['name']) {
				$directory = rtrim(DIR_IMAGE .$this->url_user() . str_replace('../', '', $this->request->post['directory']), '/');
	...
	public function delete() {
		...
		if (isset($this->request->post['path'])) {
			$path = rtrim(DIR_IMAGE .$this->url_user() . str_replace('../', '', html_entity_decode($this->request->post['path'], ENT_QUOTES, 'UTF-8')), '/');

			if (!file_exists($path)) {
				$json['error'] = $this->language->get('error_select');
			}

			if ($path == rtrim(DIR_IMAGE .$this->url_user(), '/')) {
	...
	public function move() {
		...
		if (isset($this->request->post['from']) && isset($this->request->post['to'])) {
			$from = rtrim(DIR_IMAGE .$this->url_user() . str_replace('../', '', html_entity_decode($this->request->post['from'], ENT_QUOTES, 'UTF-8')), '/');
			...
			$to = rtrim(DIR_IMAGE .$this->url_user() . str_replace('../', '', html_entity_decode($this->request->post['to'], ENT_QUOTES, 'UTF-8')), '/');
	...
	public function copy() {
		...
		if (isset($this->request->post['path']) && isset($this->request->post['name'])) {
			...
			$old_name = rtrim(DIR_IMAGE .$this->url_user() . str_replace('../', '', html_entity_decode($this->request->post['path'], ENT_QUOTES, 'UTF-8')), '/');
	...
	public function folders() {
		$this->response->setOutput($this->recursiveFolders(DIR_IMAGE .$this->url_user()));
	}

	protected function recursiveFolders($directory) {
		$output = '';

		$output .= '<option value="' . utf8_substr($directory, strlen(DIR_IMAGE .$this->url_user())) . '">' . utf8_substr($directory, strlen(DIR_IMAGE .$this->url_user())) . '</option>';
	...
	public function upload() {
		...
		$json = array();
		if($this->foldersize(DIR_IMAGE .$this->url_user())>20971520)
		$json['error'] = 'Dung luong vươt mức cho phép';
		else
		{
			if (isset($this->request->post['directory'])) {
