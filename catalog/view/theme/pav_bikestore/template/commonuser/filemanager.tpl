...
<script type="text/javascript" src="vcatalog/view/javascript/jquery/ui/external/jquery.bgiframe-2.1.2.js"></script>
<script type="text/javascript" src="catalog/view/javascript/jstree/jquery.tree.min.js"></script>
...
$(document).ready(function() { 
	...
	$('#column-right').bind('scrollstop', function() {
		$('#column-right a').each(function(index, element) {
			...
			if ((offset.top > 0) && (offset.top < height) && $(element).find('img').attr('src') == '<?php echo $no_image; ?>') {
				$.ajax({
					url: 'index.php?route=common/filemanager/image&token=<?php echo $token; ?>&image=' + encodeURIComponent('<?php echo $folder; ?>' + $(element).find('input[name=\'image\']').attr('value')),
					dataType: 'html',
	...
	$('#column-right a').live('dblclick', function() {
		...
		self.close();	
		<?php } else { ?>
		parent.$('#<?php echo $field; ?>').attr('value', '<?php $folder;  ?>' + $(this).find('input[name=\'image\']').attr('value'));
