...
class ModelCatalogProduct extends Model {
	...
	public function updateFile($filesoure,$product_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "product SET filesoure = '" . $this->db->escape(html_entity_decode($filesoure, ENT_QUOTES, 'UTF-8')) . "' WHERE product_id = '" . (int)$product_id . "'");
	}
	public function insertOption($product_id, $option_id) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "',  option_id = " .(int) $option_id);
		return $this->db->getLastId();
	}
	public function insertOptionValue($product_option_id, $product_id, $option_id,$option_value_id) {
	...
	public function IsOptions($product_id,$option_id) {	
		$option = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option   where product_id = '".(int)$product_id."' and  option_id =  ".(int)$option_id);
		
		return $option->rows;
	}
	
	public function getProductUser($user_id) {	
		$reset = $this->db->query("SELECT * FROM " . DB_PREFIX . "product   where user_id = '".(int)$user_id."'");
	...
	public function addProduct($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET  quantity = '99999999', minimum = '1',  price = '" . (float)$data['price'] . "',  status = '1',  date_added = NOW() , viewdemo = '".$data['viewdemo']."', user_id = '".$this->customer->isLogged()."'");
		...
		/* if (isset($data['filesoure'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "product SET filesoure = '" . $this->db->escape(html_entity_decode($data['filesoure'], ENT_QUOTES, 'UTF-8')) . "' WHERE product_id = '" . (int)$product_id . "'");
		} */
		if (isset($data['name'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '1', name = '" . $this->db->escape($data['name']) . "', meta_keyword = '" . $this->db->escape($data['meta_keyword']) . "', meta_description = '" . $this->db->escape($data['meta_description']) . "', description = '" . $this->db->escape($data['description']) . "'");
		}
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
                ...
                'viewdemo'           => $query->row['viewdemo'],
				'filesoure'           => $query->row['filesoure']
			);
		} else {
			return false;
		}
