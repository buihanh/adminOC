...
/**
 * class ControllerModulemegamenu 
 */
class ControllerModuleMegamenubase extends Controller {
	 ...
	 * @access private 
	 */
	private $moduleName = 'megamenubase';
	...
	public function index() {   
		...
		$this->document->addStyle('view/stylesheet/megamenubase.css');
		$this->document->addScript('view/javascript/megamenubase/jquerycookie.js');
		$this->document->addScript('view/javascript/megamenubase/jquery.nestable.js');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')  && !empty($this->request->post) ) {
			if (!$this->user->hasPermission('modify', 'module/'.$this->moduleName)) {
				...
				$this->load->model('menu/megamenu');
				$data = array();
				$data['megamenubase_module'] = $this->request->post["megamenubase_module"];
				
				$this->model_setting_setting->editSetting('megamenubase', $data);
				...
				if( isset($id) && $this->request->post['save_mode']=='save-edit'){
                    $this->redirect($this->url->link('module/megamenubase', 'id='.$id.'&token=' . $this->session->data['token'].$store_param, 'SSL'));
				}	else {
					$this->redirect($this->url->link('module/megamenubase', 'token=' . $this->session->data['token'].$store_param, 'SSL'));
				}
				$this->redirect($this->url->link('module/megamenubase', 'token=' . $this->session->data['token'].$store_param, 'SSL'));
			}
		}
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link('module/megamenubase', $url.'&token=' . $this->session->data['token'], 'SSL');
		}
		...
		$this->data['action'] = $this->url->link('module/megamenubase', 'token=' . $this->session->data['token'].$store_param, 'SSL');
		$this->data['actionGetTree'] = $this->url->link('module/megamenubase/gettree', 'token=' . $this->session->data['token'].$store_param, 'SSL');
		$this->data['actionDel'] = $this->url->link('module/megamenubase/delete', 'token=' . $this->session->data['token'].$store_param, 'SSL');
		$this->data['actionGetInfo'] = $this->url->link('module/megamenubase/info', 'token=' . $this->session->data['token'].$store_param, 'SSL');
		$this->data['updateTree'] = $this->url->link('module/megamenubase/update', 'root=1'.$store_param.'&token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['megamenubase_module'])) {
			$this->data['modules'] = $this->request->post['megamenubase_module'];
		} elseif ($this->config->get('megamenubase_module')) { 
			$this->data['modules'] = $this->config->get('megamenubase_module');
		}	
		...
		$this->template = 'module/megamenubase/megamenubase.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function delete(){
		...
		$this->redirect($this->url->link('module/megamenubase', 'token=' . $this->session->data['token'], 'SSL'));
	}
	...
	public function info(){
		...
		try{
			if($a)
			{
				...
				$this->data['blogs'] = $this->model_pavblog_menu->getDropdowns(null,$menu['item']);
			
			}
			else
			{
				$this->load->model('pavblog/menu');
				$this->data['blogs'] = $this->model_pavblog_menu->getDropdowns();
		...
		$this->template = 'module/megamenubase/megamenubase_form.tpl';
		$this->response->setOutput($this->render());
    ...
    if($results)
    {
        ...
        foreach ($results as $result) {
            $html .= "<li><a onclick='select_category(".$result['category_id'].");' href='javascript:void(0);'>".$result['name']."</a></li>";
        }
        $html .= "</ul>";
    }
    echo $html;
    ...
    function product_menu(){
        ...
        if($results)
        {
            $html .= "<ul><li>ID----- Tên sản phẩm</li>";
            foreach ($results as $result) {
                $sele = $result['product_id'].',"'.$result['name'].'"';
                $html .= "<li>".$result['product_id']."  -- <a onclick='select_product(".$sele.");' href='javascript:void(0);'>".$result['name']."</a></li>";
            }
            $html .= "</ul>";
        }
        echo $html;
	...
	protected function validate() {
		...
		if (!$this->user->hasPermission('modify', 'module/megamenubase')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['megamenubase_module'])) { 
	
			foreach ($this->request->post['megamenubase_module'] as $key => $value) {
	...
	public function addwidget(){ 
		...
		$this->language->load('module/megamenubase');
		$this->template = 'module/megamenubase/widget_form.tpl';
		$this->document->addStyle( 'view/stylesheet/megamenubase/widget.css');
		...
		if( isset($this->request->post['widget']) && isset($this->request->post['params']) ){
			...
			$this->redirect( $this->url->link('module/megamenubase/addwidget', 'done=1&id='.$row['id'].'&wtype='.$row['type'].'&token=' . $this->session->data['token'], 'SSL') ); 
		}
		...
		$this->data['action'] = $this->url->link('module/megamenubase/addwidget', 'token=' . $this->session->data['token'], 'SSL'); 
	...
	public function autocomplete(){
		...
		$arr['category_description'] = $this->model_catalog_category->getCategoryDescriptions($category_id);
		
		
		foreach($languages as $language)
		{
			$arr['languages'][] = $language['language_id'];
		}
	...
	public function informationcomplete(){
		...
		$this->load->model('catalog/information');
		$languages = $this->model_localisation_language->getLanguages();			
		$arr['information_info']  = $this->model_catalog_information->getInformationDescriptions($information_id);
	...
	/* try{
				...
				$this->load->model('pavblog/menu');
				$arr['blogs'] = $this->model_pavblog_menu->getMenuDescription( $id );
