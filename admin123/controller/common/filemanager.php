...
class ControllerCommonFileManager extends Controller {
	...
	public function url_store() {
		...
		$results = $this->model_setting_store->getStore((int)$this->user->getStoreID());
		return  'data/public/'.$results['folder'].'/';
	...
	public function index() {
		...
		$this->data['directory'] = HTTP_CATALOG . 'image/'.$this->url_store();
	...
	public function directory() {
		...
		if (isset($this->request->post['directory'])) {
			$directories = glob(rtrim(DIR_IMAGE .$this->url_store(). str_replace('../', '', $this->request->post['directory']), '/') . '/*', GLOB_ONLYDIR);
			...
			if ($directories) {
				...
				foreach ($directories as $directory) {
					...
					$json[$i]['attributes']['directory'] = utf8_substr($directory, strlen(DIR_IMAGE .$this->url_store()));
	...
	public function files() {
		...
		if (!empty($this->request->post['directory'])) {
			$directory = DIR_IMAGE . $this->url_store() . str_replace('../', '', $this->request->post['directory']);
		} else {
			$directory = DIR_IMAGE . $this->url_store();
		}
			...
			foreach ($files as $file) {
				...
				if (in_array(strtolower($ext), $allowed)) {
					...
					$json[] = array(
						...
						'file'     => utf8_substr($file, utf8_strlen(DIR_IMAGE . $this->url_store())),
	...
	public function create() {
		...
		if (isset($this->request->post['directory'])) {
			if (isset($this->request->post['name']) || $this->request->post['name']) {
				$directory = rtrim(DIR_IMAGE . $this->url_store() . str_replace('../', '', $this->request->post['directory']), '/');
	...
	public function delete() {
		...
		if (isset($this->request->post['path'])) {
			$path = rtrim(DIR_IMAGE . $this->url_store() . str_replace('../', '', html_entity_decode($this->request->post['path'], ENT_QUOTES, 'UTF-8')), '/');

			if (!file_exists($path)) {
				$json['error'] = $this->language->get('error_select');
			}

			if ($path == rtrim(DIR_IMAGE . $this->url_store(), '/')) {
	...
	public function move() {
		...
		if (isset($this->request->post['from']) && isset($this->request->post['to'])) {
			$from = rtrim(DIR_IMAGE . $this->url_store() . str_replace('../', '', html_entity_decode($this->request->post['from'], ENT_QUOTES, 'UTF-8')), '/');

			if (!file_exists($from)) {
				$json['error'] = $this->language->get('error_missing');
			}

			if ($from == DIR_IMAGE . $this->url_store()) {
				$json['error'] = $this->language->get('error_default');
			}

			$to = rtrim(DIR_IMAGE . $this->url_store() . str_replace('../', '', html_entity_decode($this->request->post['to'], ENT_QUOTES, 'UTF-8')), '/');
	...
	public function copy() {
		...
		if (isset($this->request->post['path']) && isset($this->request->post['name'])) {
			...
			$old_name = rtrim(DIR_IMAGE . $this->url_store() . str_replace('../', '', html_entity_decode($this->request->post['path'], ENT_QUOTES, 'UTF-8')), '/');
	...
	public function folders() {
		$this->response->setOutput($this->recursiveFolders(DIR_IMAGE . $this->url_store()));
	}

	protected function recursiveFolders($directory) {
		$output = '';

		$output .= '<option value="' . utf8_substr($directory, strlen(DIR_IMAGE . $this->url_store())) . '">' . utf8_substr($directory, strlen(DIR_IMAGE . $this->url_store())) . '</option>';
	...
	public function rename() {
		...
		if (isset($this->request->post['path']) && isset($this->request->post['name'])) {
			...
			if (!file_exists($old_name) || $old_name == DIR_IMAGE . $this->url_store()) {
